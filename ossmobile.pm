...
sub display
{
	...
	if( $ACTION eq 'logout' )
	{
        	sendRequest('<request name="logout" sessionID="'.$SESSIONID.'" ip="'.$this->{CGI}->remote_addr.'"/>');
	}
    	$this->login();
    }
...
sub login
{
    ...
    print		  "Benutzername:";
    print             $CGI->end_td();
    print             $CGI->start_td({-class=>'ContentValue'});
...
sub getMenu
{
    ...
    sendRequest('<request name="getMobileMenu" sessionID="'.$SESSIONID.'" ip="'.$this->{CGI}->remote_addr.'"/>');
}
...
#				  $CGI->a({-href =>'/cgi-bin/ossmobile.pl?CATEGORY='.$section.'&ACTION=SHOW_SUBMENU&SESSIONID='.$SESSIONID},menuItem($section))
#			      ));
		if( $SUB_MENU_LEFT )
		{
			foreach my $application ( @{$MENU->{$section}})
			{
				...
				{
				     $main_menu .= $CGI->Tr({}, $CGI->td({-class=>'ActivSubMenuItem'},
					  $CGI->a({-href =>'/cgi-bin/ossmobile.pl?CATEGORY='.$section.'&APPLICATION='.$application.'&ACTION=default&SESSIONID='.$SESSIONID},menuItem($section,$application))
				      ));
				}
				else
				{
...
sub EndTag
{
    ...
    Debug("EndTag ACTION '$ACTION' VARIABLE '$VARIABLE' VALUE '$VALUE'\n########\n");
    if( $v2 eq 'title' )
    {
	$TITLE=$VALUE;
	$VALUE=undef;
    ...
    elsif( $ACTION eq 'getMenu' && $VARIABLE eq 'application' )
    {
	if( $VALUE ) {
            push @{$MENU->{$CATEGORY}}, $VALUE;
